...
export const StepAmount = () => {
    const { amount, setField } = useTransactionCreateStore();
    ...
    const handleSubmit = () => {
        ...
        useTransactionCreateStore.getState().nextStep();
    };
        ...
        >
            <motion.div variants={itemVariants} className='flex gap-5 justify-center p-2'>
                <motion.button 
                    ...
                    whileTap={{ scale: 0.95 }} 
                    className={`px-6 py-1 rounded-xl transition-colors ${localType === 'income' ? 'bg-[#86CF78] text-white' : 'bg-[#374056] text-gray-400'}`}
                >
                    Income
                </motion.button>
                ...
                <motion.button 
                    ...
                    whileHover={{ scale: 1.05 }} 
                    whileTap={{ scale: 0.95 }} 
                    className={`px-6 py-1 rounded-xl transition-colors ${localType === 'expense' ? 'bg-[#FF5C5C] text-white' : 'bg-[#374056] text-gray-400'}`}
                >
                    Expense
                </motion.button>
            </motion.div>
            ...
            <motion.div variants={numpadVariants} className="grid grid-cols-3 gap-3 p-4">
                {keys.map((key) => (
                    <motion.button 
                        ...
                        whileTap={{ scale: 0.95 }} 
                        className="bg-[#374056] rounded-2xl p-4 text-lg flex items-center justify-center select-none"
                    >
                        {key.label}
                    </motion.button>
                ))}
            ...
            <motion.div variants={itemVariants} className="px-4">
                <motion.button 
                    ...
                    className={`w-full rounded-2xl p-4 text-lg flex items-center justify-center select-none ${Number(amountStr) === 0 ? 'bg-button-disabled-gradient cursor-not-allowed' : 'bg-button-gradient'}`}
                >
                    Continue
                </motion.button>
            </motion.div>
